  ...
  function generateAnalysis() {
    ...
    setTimeout(() => {
      ...
      if (tapIntervals.length >= 5) {
        const intervalStats = robustStatistics(tapIntervals, true);
        // Recalculate tap rate using robust mean interval
        if (intervalStats.mean > 0) {
          improvedTapRate = 1000 / intervalStats.mean; // taps per second
        }
        // Improve coordination score based on data quality
      ...
      results.diseaseRiskAssessment = assessMotorDiseaseRisk(
        finalTapRate, finalCoordinationScore, tremorMetrics.freqHz, tremorMetrics.ampNorm, tapIntervals
      );
      ...
      const tremorAccuracyScore = tremorQuality.sampleCount >= 10
        ? calculateAccuracyScore(robustStatistics(tremorYValues, true), tremorQuality)
        : 70;
